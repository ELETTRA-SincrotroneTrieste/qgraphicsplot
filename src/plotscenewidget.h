...
/** \brief The main class that contains the plot canvas.
  ...
  * interface that the PlotSceneWidget implements.
  *
  ...
  * PlotSceneWidget notifications when the geometry of the plot changes (see
  ...
  * <br/>
  * Other relevant changes taking place in the PlotSceneWidget are notified by means of
  ...
  *
  */
class  PlotSceneWidget : public QGraphicsView, public XYPlotInterface
{
    ...
    Q_PROPERTY(double plotAreaPercentageWidth READ plotAreaPercentageWidth WRITE setPlotAreaPercentageWidth)
    Q_PROPERTY(double plotAreaPercentageHeight READ plotAreaPercentageHeight WRITE setPlotAreaPercentageHeight)
    Q_PROPERTY(double plotAreaTopLeftXPercentage READ plotAreaTopLeftXPercentage WRITE setPlotAreaTopLeftXPercentage)
    Q_PROPERTY(double plotAreaTopLeftYPercentage READ plotAreaTopLeftYPercentage WRITE setPlotAreaTopLeftYPercentage)
...
public:
    ...
    explicit PlotSceneWidget(QWidget *parent = 0, bool useOpenGl = false);

    explicit PlotSceneWidget(QWidget *parent, bool initDefaultAxes, bool useOpenGl = false);
    ...
    double plotAreaPercentageWidth() const;

    double plotAreaPercentageHeight() const;

    double plotAreaTopLeftXPercentage() const;

    double plotAreaTopLeftYPercentage () const;
    ...
    /** \brief returns the value of the manualSceneUpdate property
      *
      * @see setManualSceneUpdate
      * @see setRefreshPeriod
      */
    ...
    /** \brief retrieve the list of curves that are closest to the viewPos point in view coordinates.
      ...
      *         the point where a click event has taken place in the PlotSceneWidget.
      *
      *
      *
...
signals:
     ...
     *         performed by the PlotSceneWidget and its zoomer scale the view
...
public slots:
    ...
    /** \brief This property sets the scene rect of the view equal to the
      *        widget geometry.
      *
      * The default is enabled. On first show event the scene rect is initialized
      * to the viewport rect.
      * The scene rect can be changed later.
      * This property is read only at the first show event: it must be changed before
      * first show event (i.e. right after object construction).
      *
      * @see sceneRectToWidgetGeometry
      */
    ...
    void setModifiedPaintEvent(bool mod);
    ...
    /* the following section configures the area of the scene occupied by the plot */

    /** \brief sets the top left point of the rectangle occupied by the plot inside the scene.
      *
      * One may want the plot to occupy only a portion of the scene, in order to place other elements
      * or drawings in the same scene. With this method you configure the top left of the rectangle
      * containing the plot. The widthPercent and heightPercent parameters are floating point values
      * between 0 and 1 and represent the percentage of the width and height of the sceneRect to be
      * considered in order to place the top left point of the plot rectangle.
      *
      * If the sceneRect width is 200 and widthPercent is 0.1, then the top left x coordinate will be
      * 20.
      *
      * @param widthPercent a number between 0 and 1 representing the percentage of the scene rect
      *        width to be considered in order to place the x coordinate of the top left point of the
      *        plot area.
      *
      * @param heightPercent a number between 0 and 1 with the same meaning of widthPercent, but
      *        referred to the scene rect height
      *
      * @see setPlotAreaPercentage
      *
      * The default values are 0.0 for both arguments, so that the plot area coincides with the
      * scene()->sceneRect() rectangle.
      *
      */
    void setPlotAreaTopLeftPercentage(double widthPercent, double heightPercent);

    /** \brief set the plot area rectangle size, in terms of percentage with respect to the scene rect
      *        area.
      *
      *
      *
      * @param widthPercent a number between 0 and 1 representing the percentage of the scene rect
      *        width to be considered in order to calculate the width of the rectangular plot area.
      *        <br/>In other words the plot area width will be equal to
      *        scene->sceneRect().width() * widthPercent
      *
      * @param heightPercent a number between 0 and 1 with the same meaning of widthPercent, but
      *        referred to the scene rect height
      *
      * @see setPlotAreaSTopLeftPercentage
      *
      * The default values are 1.0 for both arguments, so that the plot area coincides with the
      * scene()->sceneRect() rectangle.
      *
      */
    void setPlotAreaPercentage(double widthPercent, double heightPercent);

    /** \brief the same as setPlotAreaTopLeftPercentage, sets only width
      *
      */
    void setPlotAreaPercentageWidth(double widthPercent);

    /** \brief the same as setPlotAreaTopLeftPercentage, sets only height
      *
      */
    void setPlotAreaPercentageHeight(double heightPercent);

    /** \brief the same as setPlotAreaTopLeftPercentage, sets only top left x coordinate percentage
      *
      */
    void setPlotAreaTopLeftXPercentage(double xPercent);

    /** \brief the same as setPlotAreaTopLeftPercentage, sets only top left y coordinate percentage
      *
      */
    void setPlotAreaTopLeftYPercentage(double yPercent);
    ...
    /** \brief sets the QGraphicsItem flag QPainter::Antialiasing on the view
      *
      * @param en true: anti alias painting enabled
      * @param en false: anti alias painting not enabled
      *
      * The default is true. This might affect performance.
      */
    void setPainterAntiAlias(bool en);

    /** \brief sets the QGraphicsItem flag QPainter::HighQualityAntialiasing on the view
      *
      * @param en true: anti alias painting enabled
      * @param en false: anti alias painting not enabled
      *
      * The default is QGraphicsView's default.
      * This is an
      * OpenGL-specific rendering hint indicating that the engine should use
      * fragment programs and offscreen rendering for antialiasing.
      */
...
protected:
    ...
    /** \brief initializes the scene rect according to the viewport rect
      *        which is correctly initialized after the show event.
      *
      */
...
private:
    PlotSceneWidgetPrivate *d_ptr;

    void initPlot(bool useOpenGl);
